...
Module CodeBase
    ...
    Declare Function report4parent32 Lib "c4dll.dll" Alias "report4parent" (ByVal r4 As Integer, ByVal hWnd As Integer) As Short
    ...
    Function d4createData(ByVal cb As Integer, ByRef dbname As String, ByRef D() As FIELD4INFO) As Integer
        ...
        lb = LBound(D)
        ub = UBound(D)
        ' LY Apr 13/04 : changed from s = ub - 1, since Microsoft docs wrong about
        ' array sizes (i.e. dim array(N) contains elements 0 to N, not 0 to N - 1)
        s = ub

        Dim f((ub + 1)) As FIELD4INFOCB
        i = lb
        Do While D(i).fName <> "" And i <= s
            f(i).fName = v4Cstring(D(i).fName) ' note: this function allocates memory
            f(i).ftype = Asc(D(i).ftype)
            f(i).flength = D(i).flength
            f(i).fdecimals = D(i).fdecimals
    ...
    Function date4cdow(ByRef dateString_Renamed As String) As String
        ...
        'Validate "dateString"
        date4cdow = ""
        If dateString_Renamed = "" Or Len(dateString_Renamed) < 8 Then Exit Function

        Dim datePtr As Integer = date4cdowCB(dateString_Renamed) 'Get pointer to day
    ...
    Function date4cmonth(ByRef dateString_Renamed As String) As String
        ...
        Dim datePtr As Integer = date4cmonthCB(dateString_Renamed) 'Get pointer to month
    ...
    Function expr4type(ByRef exPtr As Integer) As String
        ...
        Dim exprType As Short = expr4typeCB(exPtr)
    ...
    Sub f4assignUnicode(ByRef fPtr As Integer, ByRef value As String)
        Dim bArray() As Byte = System.Text.UnicodeEncoding.Unicode.GetBytes(value & ControlChars.NullChar)
    ...
    Function f4binary(ByRef field As Integer) As Object
        Dim fLen As Integer = f4len(field)
    ...
    Function f4memoBinary(ByRef field As Integer) As Object
        Dim fLen As Integer = f4memoLen(field)
    ...
    Function f4memoStr(ByRef fPtr As Integer) As String
        ...
        If MemoLen > &H7FFFFFFF Then
            MsgBox("Error #: -910" + ControlChars.CrLf + "Unexpected Information" + vbLf + "Memo entry too long to return in a Visual Basic string." + vbCrLf + "Field Name:" + f4name(fPtr), 16, "CodeBase Error")
            Exit Function
        End If
        ...
        Dim MemoString As String = New String(" ", MemoLen)
    ...
    Sub f4memoStr64(ByRef fPtr As Integer, ByRef src As String)
        ...
        Dim r4line As String = Chr(10) & Chr(13)
        ...
        If MemoLen > r4maxVBStringLen Then
            MsgBox("Error #: -910" + vbCrLf + "Unexpected Information" + r4line + "Memo entry too long to retrieve into VB string." + r4line + "Field Name:" + f4name(fPtr), 16, "CodeBasic Error")
            Exit Sub
        End If
    ...
    Function f4name(ByRef fPtr As Integer) As String
        ...
        Dim FldNamePtr As Integer = f4nameCB(fPtr)  'Get Pointer to field name

        'Dim FldName As VB6.FixedLengthString = New VB6.FixedLengthString(11) 'String to hold info
        Dim Fldname As String = FldNamePtr = Space(11)
    ...
    Function f4nCpy(ByRef field As Integer, ByRef s As String, ByRef slen As Short) As Short
        ...
        Dim fPtr As Integer = f4ptr(field)
    ...
    Function i4create(ByVal dbPtr As Integer, ByRef IndexName As String, ByRef n() As TAG4INFO) As Integer
        ...
        If IndexName = "" Then 'User wants production index file
            i4create = i4createCB(dbPtr, 0, t(1))
        Else
    ...
    Function report4parent(ByVal r4 As Integer, ByVal hWnd As Integer) As Short
    ...
    Function t4filter(ByRef tPtr As Integer) As String
        ...
        Dim filter_Renamed As String = Space(255)
