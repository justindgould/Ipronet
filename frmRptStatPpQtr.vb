...
Friend Class frmRptStatPpQtr
    ...
    Private Sub frmRptStatPpQtr_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
...
nextrec1:
        ...
        For X = 0 To 11
            For X1 = 1 To 10
                If X1 < 6 Then B3(X, 0) = B3(X, 0) + B2(X, X1)
                If X1 = 6 Then B3(X, 1) = B3(X, 1) + B2(X, X1)
                If X1 > 6 And X1 < 11 Then B3(X, 2) = B3(X, 2) + B2(X, X1)
                If X1 < 11 Then B3(X, 3) = B3(X, 3) + B2(X, X1)
    ...
    Sub RptPageHeading()
        ...
        If H = 0 Or H = 2 Then
            ...
            prtobj.Print()
            prtobj.Print(C0str)
            prtobj.Print("Qrtly Private Passenger Report - Direct", TAB(45), Astr & "  " & A1str, TAB(121), "Page " & Pcnt)
            prtobj.Print(Z1str, TAB(45), A4str & "   " & Trim(A2str) & " thru " & J2str)
            prtobj.Print()

        End If

        If H = 0 Then prtobj.Print("Period " & J2str & " YTD")
        If H = 1 Then prtobj.Print("Period " & J3str & " YTD")
        If H = 2 Then prtobj.Print(H1)
        If H = 3 Then prtobj.Print(H1 & "Summary")
